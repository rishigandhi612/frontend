<template>
    <v-container>
      ...
      <v-card v-if="customer" class="mx-auto" max-width="600">
        <v-card-title>
        ...
        <v-card-text>
          ...
          <v-list>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title>
                  <strong>Line 1:</strong> {{ customer.address.line1 }}
                </v-list-item-title>
    ...
    methods: {
      ...
      async loadCustomerDetail() {
        try {
          ...
          // Populate updatedCustomerData with existing customer data for editing
          if (this.customer) {
            this.updatedCustomerData = { ...this.customer }; // shallow copy for editing
          }
        } catch (err) {
