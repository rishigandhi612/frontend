  ...
  <v-row class="mt-3">
    ...
    <v-col cols="12" v-if="debug">
      <div class="debug-info" style="color: red; font-size: 12px">
        Debug: GST Starts with 27: {{ gstStartsWith27 }} | GST Number:
        {{ customerGstNumber }} | Is Intra-State: {{ isIntraStateTransaction }}
      </div>
    </v-col>
...
export default {
  ...
  computed: {
    ...
    gstStartsWith27() {
      if (!this.customerGstNumber) return true;
    ...
    isIntraStateTransaction() {
      ...
      if (
        ...
        this.customerGstNumber === undefined ||
        this.customerGstNumber === " "
      ) {
        console.log(
          "Determining intra-state transaction:",
          this.customerGstNumber,
        );
        return true;
      }
  ...
  created() {
    console.log(
      "InvoiceTotals component created with customer:",
